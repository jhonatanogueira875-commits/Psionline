...
async function checkAdminStatus(userId) {
  ...
  try {
    ...
    if (error && error.code === '400') {
        // Este é o erro 400 (Bad Request) que vimos no console. 
        // Geralmente significa que o RLS está bloqueando a leitura.
...
async function handleAuthChange(session) {
  ...
  if (isAdmin) {
    ...
    currentPage = 'login';
    // Se logado mas não admin, podemos mostrar uma mensagem na tela de login
    if (session) {
...
async function handleLogin(email, password) {
  ...
  try {
    ...
    if (error) {
      ...
      return; // Permite que handleAuthChange cuide da renderização (ou a falta dela)
    }
...
function showToast(message, type = 'info') {
    ...
    toast.className = `fixed bottom-5 right-5 ${color} text-white p-4 rounded-lg shadow-xl transition-opacity duration-300`;
  ...
  return `
    <div class="flex items-center justify-center h-screen">
      <div class="glass p-8 w-full max-w-md rounded-xl shadow-2xl">
        <h1 class="text-3xl font-bold mb-6 text-gray-800 text-center">Admin Login</h1>
        <form id="login-form" class="space-y-4">
          <div>
            <label for="email" class="block text-sm font-medium text-gray-700">Email:</label>
            <input type="email" id="email" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
            <label for="password" class="block text-sm font-medium text-gray-700">Senha:</label>
            <input type="password" id="password" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div id="login-error" class="text-center"></div>
          <button type="submit" id="login-btn" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150">
            Entrar
          </button>
        </form>
      </div>
    ...
    <div class="flex h-screen bg-gray-50">
      ...
      <div class="w-64 bg-gray-800 text-white flex flex-col">
        ...
        <div class="p-4 border-t border-gray-700">
            ...
            <button id="logout-btn" class="w-full py-2 px-4 text-sm bg-red-600 hover:bg-red-700 rounded-lg transition duration-150">
                Sair
            </button>
        </div>
      </div>
...
async function renderAdminContent() {
  ...
  if (currentAdminTab === 'appointments') {
    main.innerHTML = `<div class="p-6 bg-white rounded-xl shadow">Em construção: Lista e Gerenciamento de Agendamentos.</div>`;
    return;
  }
  
  if (currentAdminTab === 'users') {
    main.innerHTML = `<div class="p-6 bg-white rounded-xl shadow">Em construção: Lista e Gerenciamento de Usuários.</div>`;
    return;
  }
}
...
function renderMetricCard(title, value, bgColor, changeText) {
  return `
    <div class="bg-white p-6 rounded-xl shadow-lg border-l-4 ${bgColor.replace('-500', '-600')}">
    ...
    try {
      ...
      const list = apps.data.map(a => {
        ...
        return `<li class="p-3 border rounded-lg mb-2 shadow-sm bg-gray-50 hover:bg-white transition duration-100">
                  ...
                  <div class="text-xs text-gray-600">${psy?.full_name || 'Psicólogo'} — ${date || '-'}</div>
                </li>`;
      }).join('');
      
      main.innerHTML = `<ul class="space-y-2">${list}</ul>`;
      
    } catch (e) {
      main.innerHTML = `<div class="p-4 text-red-700 bg-red-100 rounded-lg border border-red-300 shadow">Erro: ${e.message}</div>`;
    }
    return;
}
...
function render() {
  ...
  // Limpa listeners antigos para evitar duplicação em re-render
  document.removeEventListener('submit', attachLoginListeners); 
  document.removeEventListener('click', attachAdminListeners); 
  ...
  if (currentPage === 'admin') { 
    ...
    // É importante que o conteúdo admin seja renderizado após o shell
    // e os listeners do shell (ex: logout) estejam prontos.
    renderAdminContent(); 
    return; 
  }
...
// 1. Garante que o Chart.js está disponível
function checkChartJs() {
    return typeof window.Chart !== 'undefined';
}

// 2. Inicializa os listeners e o estado da aplicação
function initializeApp() {
  ...
  if (supabaseClient) {
    ...
    // Caso de falha de carregamento do Supabase, força tela de erro ou login
    handleAuthChange(null); 
  }

  if (!checkChartJs()) {
      // Mensagem para o caso do Chart.js não carregar no index.html
      console.error("ERRO: Chart.js não está carregado. Gráficos não funcionarão.");
  }
  
  // O onAuthStateChange é assíncrono e cuidará do render() inicial.
}
